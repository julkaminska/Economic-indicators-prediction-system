...
series = unemployment_data.values.reshape(-1, 1)

scaler = MinMaxScaler(feature_range=(0, 1))
scaled_series = scaler.fit_transform(series)
with open('../pickles/scalers/scaler_nn_unemployment.pkl', 'wb') as f:
...
def search_best_params():
    ...
    hypermodel.summary()
    #
    with open("../pickles/nn_unemployment.pkl", 'wb') as file:
        pickle.dump(hypermodel, file)
    #
    with open('../pickles/history/nn_unemployment.json', 'w') as f:
...
def unempl_model():
    ...
    plt.suptitle('Dopasowanie modelu do danych - Dane dot. stopy bezrobocia')
...
def generate_initial_files(model = None):
    ...
    with open('../pickles/metrics/nn_unemployment.json', 'w') as f:
        json.dump(summary, f)

    forecast_file = "../pickles/forecasts/unemployment_nn_forecast.csv"

    future_steps = 72
    future_predictions = predict_future(model, scaled_series, future_steps, window_size)

    last_historical = unemployment_data.index[-1] + pd.DateOffset(months=1)
...
def plot():
    ...
    future_steps = 72
    last_historical = unemployment_data.dropna().index[-1] + pd.DateOffset(months=1)
    date_range = pd.date_range(start=last_historical, periods=future_steps, freq='MS')
    ...
    plt.figure(figsize=(12, 6))
    plt.plot(unemployment_data.dropna().index[-73:], unemployment_data[-73:], label="Historyczne wartości", color='blue')
    ...
    plt.suptitle("Historyczne i prognozowane wartości dot. stopy bezrobocia")
    ...
    plt.figure(figsize=(12, 6))
    plt.plot(unemployment_data.dropna().index, unemployment_data, label="Historyczne wartości", color='blue')
    plt.plot(date_range, forecast_values, label="Prognozowane wartości", color='red')
    plt.axvline(x=unemployment_data.dropna().index[-1], linestyle='--', color='gray', label="Punkt startowy prognozy")
    plt.suptitle("Historyczne i prognozowane wartości dot. stopy bezrobocia")
    plt.title("Model sieci neuronowej")
